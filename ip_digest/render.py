...
def page(date: str, sections: dict[str, list[Item]], errors: list[str]) -> str:
    ...
    return f'<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>IP 行业快报 {date}</title>{STYLE}</head><body><main class="paper"><header class="mast"><h1>IP 行业快报</h1><p>{date} · 每日 09:00 · 个人扫盘版</p></header><nav class="deck">{nav}</nav>{"<ul class=note>本期异常信源："+error_html+"</ul>" if errors else ""}{"".join(blocks)}<footer class="note">纯规则筛选 · 原文链接仅供个人阅读</footer></main></body></html>'

def write_site(output: Path, date: str, sections: dict[str, list[Item]], errors: list[str]) -> None:
    output.mkdir(parents=True, exist_ok=True); (output / "papers").mkdir(exist_ok=True)
    content = page(date, sections, errors); (output / "index.html").write_text(content, encoding="utf-8"); (output / "papers" / f"{date}.html").write_text(content, encoding="utf-8")
